...
## 2026-08-11 — TrueRate remittance cost audit, first real run

- **Recipe:** truerate-remittance-cost-audit (new — domain mismatch disclosed; not a job-search/visa-evidence component)
- **Inputs:** `data/raw/truerate-remittance-cost-audit/run-envelope.json` (amount_usd=1000, providers=[Wise, Remitly_Economy], horizon_days=7, sample_mode=true)
- **Outputs:** `data/verified/truerate-remittance-cost-audit/{data-shape-check,quality-checked,comparison-result}.json`, `logs/truerate-remittance-cost-audit-2026-08-11.json`, `reports/generated/truerate-remittance-cost-audit-2026-08-11.md`
- **Result:** All 6 steps ran end to end with no errors. Wise estimated hidden cost 0.83% on $1,000; Remitly estimated 0.4%–1.4% (range). Timing: 85.7% of 7-day waits helped in the 14-day real sample (explicitly not generalized).
- **Open issues:** Mid-market snapshot was flagged as 2 days stale by Step 4 (staleness auto-block not yet implemented — flag only). Live rate path (`get_mid_market_rate_live`) unverified end-to-end — no network access in build sandbox to install `httpx`/test the real Frankfurter API call.
...
- **Result:** All 6 steps ran end to end with no errors. Wise estimated hidden cost 0.83% on $1,000; Remitly estimated 0.4%–1.4% (range). Timing: 85.7% of 7-day waits helped in the 14-day real sample (explicitly not generalized).
- **Open issues:** Mid-market snapshot was flagged as 2 days stale by Step 4 (staleness auto-block not yet implemented — flag only). Live rate path (`get_mid_market_rate_live`) unverified end-to-end — no network access in build sandbox to install `httpx`/test the real Frankfurter API call.
>>>>>>> f8ab629 (Add truerate-remittance-cost-audit recipe (domain mismatch disclosed in PR))
